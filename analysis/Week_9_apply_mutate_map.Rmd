...
```{r model_and_predict_h2o, eval=FALSE}
...
# explainer_penguins <- train_data %>% mutate_if(is.numeric, as.double) %>% select(-sex) %>%
#   lime::lime(
...
# 
# explanation <- test_data %>% mutate_if(is.numeric, as.double) %>% slice(1) %>% select(-sex) %>%
#   lime::explain(
