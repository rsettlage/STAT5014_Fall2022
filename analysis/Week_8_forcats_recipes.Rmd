...
penguins_new <- penguins %>%
  ...
  mutate(bill_volume_quartile = case_when(
    penguins_new$bill_volume < quantile(penguins_new$bill_volume, prob=c(0.25, 0.5, 0.75))[1] ~ "small",
    penguins_new$bill_volume < quantile(penguins_new$bill_volume, prob=c(0.25, 0.5, 0.75))[2] ~ "small-med",
    penguins_new$bill_volume < quantile(penguins_new$bill_volume, prob=c(0.25, 0.5, 0.75))[3] ~ "med-large",
    TRUE ~ "large"
  )) 
...
penguins_new <- penguins_new %>%
  mutate(bill_volume_buckets = case_when(
    penguins_new$bill_volume > quantile(penguins_new$bill_volume, prob=c(0.9, 0.8, 0.7, 0.6, 0.5, 0.4))[1] ~ "Q9",
    penguins_new$bill_volume > quantile(penguins_new$bill_volume, prob=c(0.9, 0.8, 0.7, 0.6, 0.5, 0.4))[2] ~ "Q8",
    penguins_new$bill_volume > quantile(penguins_new$bill_volume, prob=c(0.9, 0.8, 0.7, 0.6, 0.5, 0.4))[3] ~ "Q7",
    penguins_new$bill_volume > quantile(penguins_new$bill_volume, prob=c(0.9, 0.8, 0.7, 0.6, 0.5, 0.4))[4] ~ "Q6",
    penguins_new$bill_volume > quantile(penguins_new$bill_volume, prob=c(0.9, 0.8, 0.7, 0.6, 0.5, 0.4))[5] ~ "Q5",
    TRUE ~ "other"
  )) 
...
penguins_new <- penguins_new %>%
  mutate(bill_volume_buckets_fct2 = bill_volume_buckets %>% as_factor() %>% fct_rev())

# what if we want to re-order with a non sortable order?
penguins_new <- penguins_new %>%
  mutate(bill_volume_buckets_fct3 = bill_volume_buckets %>% as_factor() %>% fct_rev() %>% fct_relevel("other", after=Inf))
...
```{r model_and_predict_h2o, eval=TRUE, echo=TRUE, include=TRUE}
library(h2o)
h2o.init()
...
StackedEnsemble_BestOfFamily_6_AutoML_4_20221025_211615 <- h2o.getModel("StackedEnsemble_BestOfFamily_6_AutoML_4_20221025_211615")

predictions <- h2o.predict(StackedEnsemble_BestOfFamily_6_AutoML_4_20221025_211615, newdata = test_h2o) %>% as.tibble() %>% bind_cols(test_data_baked %>% select(flipper_length_mm, sex_male)) 
...
predictions_train_data <- h2o.predict(StackedEnsemble_BestOfFamily_6_AutoML_4_20221025_211615, newdata = train_h2o) %>% as.tibble() %>% bind_cols(train_data_baked %>% select(flipper_length_mm, sex_male)) 
